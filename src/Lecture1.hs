...
minmax x y z = ma - mi
    where ma = max (max x y) z
          mi = min (min x y) z

{- let in case
minmax x y z = 
    let ma = max (max x y) z
        mi = min (min x y) z
    in ma - mi -}
...
lowerAndGreater n (x:xs) 
                | n > x = lowerAndGreater n xs --count n is greater than one element
                | n < x = lowerAndGreater n xs --count n is lower than one element
